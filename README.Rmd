...
The default behaviour will be to take the rate to be the product of all factors specified.  
...
Or equivalently, in this simple case where we have just one variable indicating a single set of sub-populations (e.g., different age groups), then we could run `dgnpop()` on each sub-population individually, and aggregate up:  
```{r}
eg4.5 |> 
  nest(data=-agegroup) |>
  mutate(
...
However, it may be something more complex. For instance, when working with vector factors, we might define the rate as in various ways that include aggregating over sub-populations in multiple combinations of factors (e.g., `sum(a*b)/sum(a*b*c)`).  
...
Very often, the splitting up a population into various sub-populations is because we are interested in separating out how much the population rate differences are due to difference in the compositional structure of the population vs differences in the cell-specific rates.  

To do this, we require data on the sizes of each sub-population. The simplest case here would be to disagreggate into a single set of sub-populations (e.g., age-groups), and simply have cell-specific rates themselves. The crude rates for the population are therefore simply the sum of all the cell-specific rates weighted by the relative size of the cell.  
...
2. We could alternatively simply include the conversion to proportion in the rate function:  
```{r}
...
3. In addition, we can provide the variable indicating the size of each sub-population into the `crossclassified` argument of `dgnpop()`. 
```{r}
