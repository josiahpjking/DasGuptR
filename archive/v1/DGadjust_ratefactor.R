...
DGadjust_ratefactor<-function(df2,pop,i,factrs,ratefunction){
  ...
  allperms<-combn(allfacts[!(allfacts %in% c(facti,paste0(facti,1)))],length(allfacts)/2-1) %>%
  ...
  allperms %>% mutate(
    ...
    eqp=map_dbl(c0s, ~nfact*max(dim(combn(nfact-1,.)))),
  ...
  #ratefunction2 = gsub(facti,"1",ratefunction)
